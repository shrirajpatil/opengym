...
export default function RestTimer() {
  ...
  const flashId = useUI(s => s.timerFlashId)
  ...
  // TimerFlash.jsx owns the "time's up" alert (a whole-app theme blink). This just adds a
  // small acknowledgement on the clock itself the instant that fires, for the moment right
  // after — flashId only changes when a countdown actually completes, never on every tick.
  const justFinished = flashId > 0 && !!on
  if (!on) return null
  const pct = (on.left / on.total) * 100

  if (work) return (
    <div id="timer" className="working">
      <div className={'t' + (justFinished ? ' pulse-once' : '')} key={flashId}>{clock(work.left)}</div>
      <div className="grow">
  ...
  return (
    <div id="timer" className="rest">
      <div className="head">
        <div className={'t' + (justFinished ? ' pulse-once' : '')} key={flashId}>{clock(timer.left)}</div>
        <div className="bar"><i style={{ width: pct + '%' }} /></div>
      </div>
      <div className="acts">
